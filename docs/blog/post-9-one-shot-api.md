<div class="abstract" id="org1dbe8eb">
<p>
...
The key is that the *source text* is a template argument. `source_literal<N>` is a struct that stores a null-terminated copy of the text in a fixed char array, which makes it usable as a non-type template parameter &#x2014; so a program's content, not just its address or length, can drive template argument deduction.
...
That literal is the key to keying a variable template on the program's text &#x2014; the whole pipeline runs once, at namespace-scope `constexpr` initialization, and the result is stored in a `forth_program` that offers `.run()`, `.stack()`, and `.output()`:
...
Cleaning up around this, the placeholder entry point that used to return the string `"Steve"` is gone, and the library is header-only &#x2014; nothing to link, just include and instantiate.
