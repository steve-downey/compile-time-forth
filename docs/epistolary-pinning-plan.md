...
```txt
REPO        absolute path to the primary worktree (e.g. ~/src/<project>/main)
POSTS       glob of post files (e.g. docs/blog/post-*.org)
LIVING      living docs that keep worktree resolution (e.g. docs/compiler_architecture.org)
TAGPREFIX   tag namespace (default: blog/ ; multi-series repos: blog/<series>/)
INIT        the batch-export init file loaded by `make blog-md`
LISPDIR     where the module lands (e.g. etc/blog/ or lisp/)
CONTRACT    the blog authoring contract file (e.g. docs/blog/AGENTS.md)
```
