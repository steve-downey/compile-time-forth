...
- A base exposes every operation as its own member. Never `using Impl::op;` in a base — that re-forms the inheritance chain and breaks two-deep composition, because the inner base's `impl_of` then addresses the outer wrapper. The map that closes the chain (`parser_ops`, `result_functor_map`) may re-expose its Impl's primitives with using-declarations; that is where the kit's own instance maps do it.
...
- These rules are the kit's (`~/src/compile-time-scheme/main/docs/CODING_RULES.md`, Typeclass Design); `src/smd/forth/foundation/` is a mirror of that kit and inherits them unchanged. Change them there, then re-run `scripts/sync-kit.py`.
...
- The unit is `identity()`, not `empty()`. `empty` belongs to Foldable, as the predicate. For the same reason Alternative's identity is `zero()`.
