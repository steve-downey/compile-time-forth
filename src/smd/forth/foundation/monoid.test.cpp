// src/smd/forth/foundation/monoid.test.cpp                          -*-C++-*-
// SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception
// Adapted by copy from compile-time-scheme (smd::kit::foundation):
// src/smd/kit/foundation/monoid.test.cpp
// Moved in step R8 (decision 2, corrected) from
// src/smd/cl/foundation/monoid.test.cpp.

#include <smd/forth/foundation/monoid.hpp>
#include <smd/forth/foundation/monoid.hpp> // test 2nd include OK
...
using smd::forth::foundation::all;
using smd::forth::foundation::all_monoid;
using smd::forth::foundation::any;
using smd::forth::foundation::any_monoid;
using smd::forth::foundation::maximum;
using smd::forth::foundation::maximum_monoid_t;
using smd::forth::foundation::minimum;
using smd::forth::foundation::minimum_monoid_t;
using smd::forth::foundation::monoid;
using smd::forth::foundation::monoid_for;
using smd::forth::foundation::product;
using smd::forth::foundation::sum;
using smd::forth::foundation::sum_monoid;
using smd::forth::foundation::unit;
using smd::forth::foundation::unit_monoid;
...
static_assert(!smd::forth::foundation::has_instance_v<decltype(monoid<int>)>);
static_assert(!smd::forth::foundation::has_instance_v<decltype(monoid<long>)>);
static_assert(
    !smd::forth::foundation::has_instance_v<decltype(monoid<std::size_t>)>);
static_assert(!smd::forth::foundation::has_instance_v<decltype(monoid<bool>)>);
static_assert(
    !smd::forth::foundation::has_instance_v<decltype(monoid<double>)>);

// --- The named carriers, each registered under its own name. --------------

static_assert(smd::forth::foundation::has_instance_v<decltype(monoid<unit>)>);
static_assert(
    smd::forth::foundation::has_instance_v<decltype(monoid<sum<int>>)>);
static_assert(
    smd::forth::foundation::has_instance_v<decltype(monoid<product<int>>)>);
static_assert(smd::forth::foundation::has_instance_v<decltype(monoid<any>)>);
static_assert(smd::forth::foundation::has_instance_v<decltype(monoid<all>)>);
